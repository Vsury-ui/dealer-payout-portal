...
export default function UploadHistoryPage() {
  ...
  const fetchUploadHistory = async () => {
    try {
      const response = await fetch('http://localhost:3001/api/users/upload-history', {
        headers: {
  ...
  const viewJobDetails = async (jobId: string) => {
    try {
      const response = await fetch(`http://localhost:3001/api/users/upload-history/${jobId}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
  ...
  return (
    <div>
      <div className="mb-8 flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Bulk Upload History</h1>
          <p className="mt-2 text-gray-600">Track and review all bulk user upload jobs</p>
        </div>
        <button
